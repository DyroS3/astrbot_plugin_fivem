...
- **后台循环恢复能力**：修复定时推送任务异常退出后无法自动恢复的问题，单次异常不再打死整个后台轮询链路
- **通知模板安全渲染**：服务器通知与离线告警模板改为安全占位符替换，错误占位符或孤立花括号不再导致推送崩溃
- **初始化回滚**：Webhook 启动失败时回收已创建的 session / 后台任务，避免插件部分初始化成功后遗留资源
- **外部数据容错**：补强 `/status`、`/players`、`/job`、管理 API 响应的结构校验与降级处理，避免异常数据触发运行时错误
- **自检语义修正**：`/fivem 自检` 现在区分运行故障与配置提醒，避免把未启用功能或未订阅状态误判为异常

### 改进

- **Markdown 转义增强**：玩家名、职业名、服务器名等动态字段在表格渲染前进行转义，降低卡片排版被特殊字符破坏的概率
- **重启倒计时显示修正**：计划重启通知中的分钟数改为向上取整，避免出现“剩余 0 分钟”的误导显示
- **多平台目标解析优化**：新增 `push.default_platform_id` 配置项，当 `push_targets` 使用纯群号且 AstrBot 存在多个平台适配器时，可显式指定构造 UMO 使用的平台 ID
- **可观测性增强**：`/fivem 自检` 与 `/fivem 订阅列表` 现在展示后台任务用途和推送目标连续失败次数，便于长期运维排障
- **后台轮询说明补充**：在配置文档中明确 `alert_enabled` 会触发按 `auto_push_interval` 的后台 `/status` 轮询，用于离线检测与趋势采集
- **文档同步**：更新根 README、插件 README、版本号与元数据，修正文转图依赖说明和 LLM 工具数量描述

## v1.15.2

### 修复

- **趋势图 emoji 损坏**：修复 `tool_trend` 中 `📊` 字符损坏的问题

### 改进

- **趋势图 SVG 放大**：图表尺寸从 440×180 放大至 1060×360，线条加粗、圆点放大、标签字号增大，适配 1280px 视口
- **DRY 重构**：抽取 `_ensure_session()` 方法，消除 `_request` / `_post_admin` 中 session 重建的重复代码
- **清理废弃文件**：移除不再被引用的 `templates.py`（旧版 HTML 图片卡片模板）

## v1.15.1

### 改进

- **通知推送回归纯文本**：服务器事件通知（启动/关闭/重启/公告/告警）改回纯文本推送，QQ 消息列表直接可读，无需打开图片
- **全局 t2i 模板字号放大**：基于 1280px 视口重新设计，正文 38px、H1 64px、H2 48px，QQ 聊天缩略图中可直接阅读
...
- 服务器通知与离线告警改用 `text_to_image()` 渲染，高度自适应内容，消除底部大面积空白
  - 根因：`html_render()` 受 AstrBot t2i 服务视口高度硬编码 720px 限制
  - 方案：改用 AstrBot 内置 `text_to_image()` Markdown 文转图，自动适配内容高度

## v1.14.0

### 新功能

- **服务器通知图片卡片**：服务器事件通知（启动/关闭/重启/公告/告警）渲染为精美深色图片卡片
  - 不同事件类型自动匹配颜色主题（绿色=启动、红色=关闭/告警、橙色=重启、蓝色=公告）
  - `{at_all}` 占位符触发 @全体成员与图片在同一条消息中发送
  - 渲染失败自动回退纯文本，玩家动态保持纯文本推送
...
- **AI 自然语言查询**：利用 AstrBot LLM 框架，用户可用自然语言提问而无需记忆命令
  - 注册 5 个 LLM 工具：服务器状态、玩家列表、职业查询、玩家搜索、趋势图
